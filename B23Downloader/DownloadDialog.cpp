...
static QnList qnListDifference(QnList a, QnList b)
{
    ...
    std::sort(a.begin(), a.end(), std::greater<QnList::iterator::value_type>());
    std::sort(b.begin(), b.end(), std::greater<QnList::iterator::value_type>());
...
void DownloadDialog::updateQnComboBox(QnList qnList)
{
    ...
    if (contentType == ContentType::Live) {
        for (auto qn : qnList) {
            qnDescList.append(std::make_pair(qn, LiveDownloadTask::getQnDescription(qn)));
        }
        if (!onlyOneItem) {
        ...
        for (auto qn : qnList) {
            qnDescList.append(std::make_pair(qn, VideoDownloadTask::getQnDescription(qn)));
        }
        if (!onlyOneItem) {
